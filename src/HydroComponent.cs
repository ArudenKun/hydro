...
public abstract class HydroComponent : TagHelper, IViewContextAware
{
    ...
    private static readonly MethodInfo InvokeActionMethod = typeof(HydroComponent).GetMethod(
        nameof(InvokeAction),
        BindingFlags.Static | BindingFlags.NonPublic
    );

    private static readonly MethodInfo InvokeActionAsyncMethod = typeof(HydroComponent).GetMethod(
        nameof(InvokeActionAsync),
        BindingFlags.Static | BindingFlags.NonPublic
    );
    ...
    private static readonly ConcurrentDictionary<
        Type,
        IHydroAuthorizationFilter[]
    > ComponentAuthorizationAttributes = new();
    ...
    private void ConfigurePolls()
    {
        ...
        if (methods.Any(p => p.ParametersCount != 0))
        {
            throw new InvalidOperationException(
                "Poll can be defined only on actions without parameters"
            );
        }
        ...
        var polls = methods
            .Select(p => new HydroPoll(
                p.Method.Name,
                TimeSpan.FromMilliseconds(p.Attribute.Interval)
            ))
            .ToList();
    ...
    public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        _logger =
            services.GetService<ILogger<HydroComponent>>() ?? NullLogger<HydroComponent>.Instance;

        if (_persistentState == null || _options == null)
        {
            throw new ApplicationException(
                "Hydro has not been initialized with UseHydro in the application startup."
            );
        }
    ...
    )
    {
        var operationId =
            ...
            && HttpContext.Request.Headers.TryGetValue(
                HydroConsts.RequestHeaders.OperationId,
                out var incomingOperationId
            )
                ? incomingOperationId.First()
                : Guid.NewGuid().ToString("N");

        _dispatchEvents.Add(
            new HydroComponentEvent
            {
                Name = name,
                Data = data,
    ...
    /// <param name="asynchronous">Do not chain the execution of handlers and run them separately</param>
    /// <typeparam name="TEvent">Event type</typeparam>
    public void DispatchGlobal<TEvent>(
        TEvent data,
        string subject = null,
        bool asynchronous = false
    ...
    public void Redirect(string url) => HttpContext.Response.HydroRedirect(url);
    ...
    private async Task<string> GenerateComponentHtml(
        string componentId,
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException(
                "The wire component must have only one root element."
            );
        }
        ...
        rootElement.AppendChild(
            GetModelScript(componentHtmlDocument, componentId, persistentState)
        );

        foreach (var subscription in _subscriptions)
        {
            rootElement.AppendChild(
                GetEventSubscriptionScript(componentHtmlDocument, subscription)
            );
        }
        ...
        if (includeScripts)
        {
            ...
            {
                AddClientScript(
                    $"Hydro.dispatchEvents('{GetSerializedEventDispatchers()}', this);"
                );
            }
    ...
    private async Task BindModel(IFormCollection formCollection)
    {
        ...
        foreach (var pair in formCollection)
        {
            ...
            {
                var value =
                    setter.Value.Value == null ? null
                    : _options.ValueMappersDictionary.TryGetValue(
                        setter.Value.Value.GetType(),
                        out var mapper
                    )
    ...
    private HtmlNode GetModelScript(
    ...
    )
    {
        ...
        var serializeDeclaredProperties = PropertyInjector.SerializeDeclaredProperties(
            GetType(),
            this
        );
    ...
    private HtmlNode GetEventSubscriptionScript(
        HtmlDocument document,
        HydroEventSubscription subscription
    )
    {
        var eventData = new
        {
        ...
        scriptNode.SetAttributeValue(
            "x-on-hydro-event",
            JsonSerializer.Serialize(eventData, JsonSerializerSettings)
        );
        return scriptNode;
    }
    ...
    private HtmlNode GetPollScript(HtmlDocument document, HydroPoll poll, int index)
    {
        var scriptNode = document.CreateElement("script");
        scriptNode.SetAttributeValue(
            $"x-hydro-polling.{poll.Interval.TotalMilliseconds}ms._{index}",
            poll.Action
        );
        scriptNode.SetAttributeValue("type", "text/hydro");
        return scriptNode;
    ...
    private HtmlNode GetClientScript(HtmlDocument document, string script)
    {
        ...
        scriptNode.InnerHtml =
            $"Hydro.executeComponentJs('{_componentId}', function() {{ {script} }});";
        return scriptNode;
    }
    ...
    private void PopulateDispatchers()
    {
        ...
        HttpContext.Response.Headers.TryAdd(
            HydroConsts.ResponseHeaders.Trigger,
            GetSerializedEventDispatchers()
        );
    }
    ...
    private void PopulateClientScripts()
    {
        ...
        HttpContext.Response.Headers.TryAdd(
            HydroConsts.ResponseHeaders.Scripts,
            Base64.Serialize(_clientScripts)
        );
    ...
    {
        var renderedComponentIds = (string[])
            HttpContext.Items[HydroConsts.ContextItems.RenderedComponentIds];
    ...
    {
        if (
            !HttpContext.Items.TryGetValue(HydroConsts.ContextItems.MethodName, out var method)
            || method is not string methodValue
        ...
        var requestParameters =
            (IDictionary<string, object>)HttpContext.Items[HydroConsts.ContextItems.Parameters];

        var methodInfo = methodInfos.FirstOrDefault(m =>
            string.Equals(m.Name, methodValue, StringComparison.OrdinalIgnoreCase)
            && m.GetParameters()
                .Select(p => p.Name)
                .SequenceEqual(requestParameters.Select(p => p.Key))
        );
        ...
        if (
            requestParameters.Count != methodParameters.Length
        ...
        if (typeof(Task).IsAssignableFrom(methodInfo.ReturnType))
        {
            ...
            {
                var result = await (Task<IComponentResult>)
    ...
    public T GetPayload<T>() =>
        HttpContext.Request.Headers.TryGetValue(
            HydroConsts.RequestHeaders.Payload,
            out var payloadString
        )
    ...
    {
        if (
            !HttpContext.Items.TryGetValue(HydroConsts.ContextItems.EventName, out var eventName)
            || eventName is not string eventNameValue
            || string.IsNullOrWhiteSpace(eventNameValue)
        )
        {
            return;
        }
        ...
        var subscriptions = _subscriptions
            .Where(s =>
                s.EventName == eventNameValue
                && (s.SubjectRetriever == null || s.SubjectRetriever() == eventSubject)
            )
            .ToList();
        ...
        foreach (var subscription in subscriptions)
        {
            ...
            var model = Base64.Deserialize(
                (string)HttpContext.Items[HydroConsts.ContextItems.EventData],
                parameterType
            );
            ...
            HttpContext.Response.Headers.TryAdd(
                HydroConsts.ResponseHeaders.OperationId,
                operationId
            );
    ...
    {
        if (
            HttpContext.Items.TryGetValue(HydroConsts.ContextItems.RequestForm, out var requestForm)
        )
        {
    ...
    private string GetViewPath() => ViewPath ?? GetViewPath(GetType());
            ...
            else
            {
                ...
                catch
                {
                    throw new InvalidCastException(
                        $"Type mismatch in {sourceProperty.Name} parameter."
                    );
                }
            }
    ...
    private bool ValidateTouched()
    {
        ...
        foreach (var validationResult in extractValidationResults)
        {
            foreach (var memberName in validationResult.MemberNames)
            {
                if (
                    IsModelTouched
                    || TouchedProperties.Contains(memberName)
                    || TouchedProperties.Any(p => p.StartsWith($"{memberName}."))
                )
                {
    ...
    private static IEnumerable<ValidationResult> ExtractValidationResults(
    ...
    private async Task<bool> AuthorizeAsync()
    {
        ...
        {
            ComponentAuthorizationAttributes.TryAdd(
                type,
    ...
    internal void AddClientScript(string script) => _clientScripts.Add(script);
